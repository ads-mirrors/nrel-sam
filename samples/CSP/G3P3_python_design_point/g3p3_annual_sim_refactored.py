...
def run_g3p3_case(P_ref_MWe, design_eff, T_htf_cold_des_C, T_htf_hot_des_C,             
                  plant_spec_cost_per_kWe, W_dot_cycle_parasitic_input_MWe, 
                  ssc_sim,
    ...
    json_file_path = os.path.join(script_dir, "sam_gen3_particle_for_tmb.json")    
    sam_g3p3_dict = json.load(open(json_file_path, 'r'))
    ...
    # Set system design parameters based on cycle - these should NOT be defined in JSON
    sam_g3p3_dict["P_ref"] = P_ref_MWe        # MWe this should be cycle output *before* cooling parasitics
    sam_g3p3_dict["design_eff"] = design_eff  # similarly this should be cycle output before cooling parasitics
    sam_g3p3_dict["T_htf_cold_des"] = T_htf_cold_des_C    # C
    sam_g3p3_dict["T_htf_hot_des"] = T_htf_hot_des_C     # C
    sam_g3p3_dict["plant_spec_cost"] = plant_spec_cost_per_kWe  # $/kWe where kWe is = P_ref
    W_dot_cycle_parasitic_input = W_dot_cycle_parasitic_input_MWe   # MWe - Not used in current cmod setup, but need to track for net parasitics below
    ...
    design_flux_kwm2 = 1400     #kW/m2  (was 500 before)
    ...
    sam_g3p3_dict["fp_rec_height"] = [side_rec]
    sam_g3p3_dict["rec_width"] = [side_rec]

    # Change Receiver settings to make constant efficiency
    sam_g3p3_dict["rec_model_type"] = 0     # FIXED efficiency
    sam_g3p3_dict["rec_eta_fixed"] = 0.83   # Fixed efficiency

    # Overwrite variables that are passed in 
    for key in overwrite_dict:
        sam_g3p3_dict[key] = overwrite_dict[key]

    is_financial = False
    g3p3_so_solved_dict = ssc_sim.cmod_particle_from_dict(sam_g3p3_dict, is_financial, is_ssc_print=not suppress_print)  # ssc_sim.cmod_mspt_from_dict(sam_mspt_dict)

    is_success = g3p3_so_solved_dict["cmod_success"]    
    print ('SSC simulation(s) successful = ', is_success)

    output_labels = [['P_ref'], ['design_eff'], ['T_htf_cold_des'], ['T_htf_hot_des'], ['plant_spec_cost'],         # PC
                     ['m_dot_htf_cycle_des'], ['q_dot_cycle_des'], ['W_dot_cycle_lift_des'], 
                     ['W_dot_cycle_cooling_des'], 
                     ['A_sf', 0], ['solarm'],                                                                   # Field
                     ['q_dot_rec_des_total'], ['eta_rec_thermal_des'], ['P_tower_lift_des'], ['Q_transport_loss_des'],    # Receiver
                     ['m_dot_htf_rec_des'], ['h_tower_calc'], ['rec_width_calc', 0], 
                     
                     ['Q_tes_des'], ['V_tes_htf_total_des'],                                                        # TES
                     ['nameplate'], ['W_dot_bop_design'], ['W_dot_fixed'],                                          # BOP
                     
                     ['csp.pt.cost.site_improvements'], ['csp.pt.cost.heliostats'], ['csp.pt.cost.tower'],          # Costs
                     ['csp.pt.cost.receiver'], ['receiver_lift_cost'], ['csp.pt.cost.storage'], 
                     ['tes_medium_cost'], ['tes_bin_cost'], ['tes_lift_cost'], ['phx_lift_cost'],
                     ['csp.pt.cost.power_block'], ['csp.pt.cost.bop'], ['csp.pt.cost.fossil'], ['ui_direct_subtotal'], 
                     ['csp.pt.cost.contingency'], ['total_direct_cost'], ['csp.pt.cost.epc.total'], ['total_installed_cost'],
                     
                     ['csp.pt.cost.plm.total'], ['csp.pt.cost.sales_tax.total'],

                     ['contingency_rate'], ['total_land_area']
                     ]
    ...
    if (is_success == 1):
        ...
        # Design Point
            ...
            # receiver
            q_dot_rec_des = g3p3_so_solved_dict["q_dot_rec_des_total"]
            ...
            W_dot_net_ish = W_dot_cycle_des - W_dot_cycle_parasitic_input - W_dot_cycle_lift_des - (P_tower_lift_des / sam_g3p3_dict['solarm'])

            # ^ don't want gross plant output
            # no heliostat parasitics, no bop
            # Tracking isn't really a design point calc
            ...
            # Collect results
            for label_vec in output_labels:
                if(len(label_vec) == 1):
                    output_dict[label_vec[0]] = g3p3_so_solved_dict[label_vec[0]]
                else:
                    output_dict[label_vec[0]] = g3p3_so_solved_dict[label_vec[0]][label_vec[1]]

            # Get all cost results
            for key in g3p3_so_solved_dict:
               if (key in output_dict) == False:
                   if 'cost' in key:
                       output_dict[key] = g3p3_so_solved_dict[key] 

            # Collect unique results
            output_dict['q_dot_rec_per_A_sf'] = q_dot_rec_per_A_sf
            output_dict['W_dot_net_ish'] = W_dot_net_ish
            output_dict['cost_per_kWe_gross'] = cost_per_kWe_gross
            output_dict['cost_per_kWe_net_ish'] = cost_per_kWe_net_ish
    ...
    output_dict['cmod_success'] = g3p3_so_solved_dict['cmod_success']
    return output_dict
