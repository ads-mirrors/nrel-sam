...
SAM_EXPORT void SAM_Saleleaseback_Revenue_dispatch_factors_ts_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "dispatch_factors_ts", arr, length);
	});
}

SAM_EXPORT void SAM_Saleleaseback_Revenue_dispatch_sched_weekday_mset(SAM_table ptr, double* mat, int nrows, int ncols, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_matrix(ptr, "dispatch_sched_weekday", mat, nrows, ncols);
	});
}

SAM_EXPORT void SAM_Saleleaseback_Revenue_dispatch_sched_weekend_mset(SAM_table ptr, double* mat, int nrows, int ncols, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_matrix(ptr, "dispatch_sched_weekend", mat, nrows, ncols);
	});
}

SAM_EXPORT void SAM_Saleleaseback_Revenue_dispatch_tod_factors_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
...
SAM_EXPORT void SAM_Saleleaseback_Revenue_ppa_multiplier_model_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
...
SAM_EXPORT void SAM_Saleleaseback_Revenue_start_day_of_year_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
...
SAM_EXPORT void SAM_Saleleaseback_BatterySystem_en_wave_batt_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "en_wave_batt", number);
	});
}

SAM_EXPORT void SAM_Saleleaseback_FuelCell_annual_fuel_usage_lifetime_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "annual_fuel_usage_lifetime", arr, length);
	});
}

SAM_EXPORT void SAM_Saleleaseback_FuelCell_fuelcell_annual_energy_discharged_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "fuelcell_annual_energy_discharged", arr, length);
	});
}

SAM_EXPORT void SAM_Saleleaseback_FuelCell_fuelcell_replacement_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "fuelcell_replacement", arr, length);
	});
}

SAM_EXPORT double* SAM_Saleleaseback_Revenue_dispatch_factors_ts_aget(SAM_table ptr, int* length, SAM_error *err){
...
SAM_EXPORT double* SAM_Saleleaseback_Revenue_dispatch_sched_weekday_mget(SAM_table ptr, int* nrows, int* ncols, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
...
SAM_EXPORT double* SAM_Saleleaseback_Revenue_dispatch_sched_weekend_mget(SAM_table ptr, int* nrows, int* ncols, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
...
SAM_EXPORT double* SAM_Saleleaseback_Revenue_dispatch_tod_factors_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "dispatch_tod_factors", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "dispatch_tod_factors");
	});
	return result;
}
...
SAM_EXPORT double SAM_Saleleaseback_Revenue_ppa_multiplier_model_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
...
SAM_EXPORT double SAM_Saleleaseback_Revenue_start_day_of_year_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "start_day_of_year", &result))
		make_access_error("SAM_Saleleaseback", "start_day_of_year");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Saleleaseback_ElectricityRates_rate_escalation_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Saleleaseback", "rate_escalation");
	});
	return result;
}
...
SAM_EXPORT double SAM_Saleleaseback_BatterySystem_en_wave_batt_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "en_wave_batt", &result))
		make_access_error("SAM_Saleleaseback", "en_wave_batt");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_FuelCell_annual_fuel_usage_lifetime_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "annual_fuel_usage_lifetime", length);
	if (!result)
...
SAM_EXPORT double* SAM_Saleleaseback_FuelCell_fuelcell_annual_energy_discharged_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
...
SAM_EXPORT double* SAM_Saleleaseback_FuelCell_fuelcell_replacement_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "fuelcell_replacement", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "fuelcell_replacement");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_purchases_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_purchases", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_purchases");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_apr_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_apr", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_apr");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_aug_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_aug", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_aug");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_dec_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dec", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_dec");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_dispatch1_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dispatch1", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_dispatch1");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_dispatch2_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dispatch2", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_dispatch2");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_dispatch3_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dispatch3", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_dispatch3");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_dispatch4_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dispatch4", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_dispatch4");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_dispatch5_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dispatch5", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_dispatch5");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_dispatch6_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dispatch6", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_dispatch6");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_dispatch7_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dispatch7", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_dispatch7");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_dispatch8_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dispatch8", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_dispatch8");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_dispatch9_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dispatch9", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_dispatch9");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_feb_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_feb", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_feb");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_jan_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_jan", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_jan");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_jul_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_jul", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_jul");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_jun_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_jun", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_jun");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_mar_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_mar", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_mar");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_may_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_may", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_may");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_monthly_firstyear_TOD1_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_monthly_firstyear_TOD1", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_monthly_firstyear_TOD1");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_monthly_firstyear_TOD2_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_monthly_firstyear_TOD2", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_monthly_firstyear_TOD2");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_monthly_firstyear_TOD3_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_monthly_firstyear_TOD3", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_monthly_firstyear_TOD3");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_monthly_firstyear_TOD4_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_monthly_firstyear_TOD4", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_monthly_firstyear_TOD4");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_monthly_firstyear_TOD5_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_monthly_firstyear_TOD5", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_monthly_firstyear_TOD5");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_monthly_firstyear_TOD6_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_monthly_firstyear_TOD6", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_monthly_firstyear_TOD6");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_monthly_firstyear_TOD7_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_monthly_firstyear_TOD7", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_monthly_firstyear_TOD7");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_monthly_firstyear_TOD8_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_monthly_firstyear_TOD8", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_monthly_firstyear_TOD8");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_monthly_firstyear_TOD9_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_monthly_firstyear_TOD9", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_monthly_firstyear_TOD9");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_nov_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_nov", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_nov");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_oct_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_oct", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_oct");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_sales_sep_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_sep", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_sales_sep");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_energy_without_battery_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_without_battery", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_energy_without_battery");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_om_capacity1_expense_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_om_capacity1_expense", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_om_capacity1_expense");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_om_capacity2_expense_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_om_capacity2_expense", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_om_capacity2_expense");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_om_fixed1_expense_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_om_fixed1_expense", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_om_fixed1_expense");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_om_fixed2_expense_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_om_fixed2_expense", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_om_fixed2_expense");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_om_production1_expense_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_om_production1_expense", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_om_production1_expense");
	});
	return result;
}

SAM_EXPORT double* SAM_Saleleaseback_Outputs_cf_om_production2_expense_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_om_production2_expense", length);
	if (!result)
		make_access_error("SAM_Saleleaseback", "cf_om_production2_expense");
	});
	return result;
}
...
SAM_EXPORT double SAM_Saleleaseback_Outputs_pre_depr_alloc_basis_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "pre_depr_alloc_basis", &result))
		make_access_error("SAM_Saleleaseback", "pre_depr_alloc_basis");
	});
	return result;
}

SAM_EXPORT double SAM_Saleleaseback_Outputs_pre_itc_qual_basis_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "pre_itc_qual_basis", &result))
		make_access_error("SAM_Saleleaseback", "pre_itc_qual_basis");
	});
	return result;
}
