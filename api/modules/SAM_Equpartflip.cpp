...
SAM_EXPORT void SAM_Equpartflip_Revenue_start_day_of_year_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
...
SAM_EXPORT void SAM_Equpartflip_FinancialParameters_batt_salvage_percentage_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
...
SAM_EXPORT void SAM_Equpartflip_SystemCosts_om_batt_capacity_cost_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "om_batt_capacity_cost", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_SystemCosts_om_batt_fixed_cost_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "om_batt_fixed_cost", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_SystemCosts_om_batt_nameplate_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "om_batt_nameplate", number);
	});
}

SAM_EXPORT void SAM_Equpartflip_SystemCosts_om_batt_replacement_cost_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "om_batt_replacement_cost", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_SystemCosts_om_batt_variable_cost_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
...
SAM_EXPORT void SAM_Equpartflip_SystemCosts_om_fuelcell_capacity_cost_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "om_fuelcell_capacity_cost", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_SystemCosts_om_fuelcell_fixed_cost_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "om_fuelcell_fixed_cost", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_SystemCosts_om_fuelcell_nameplate_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "om_fuelcell_nameplate", number);
	});
}

SAM_EXPORT void SAM_Equpartflip_SystemCosts_om_fuelcell_replacement_cost_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "om_fuelcell_replacement_cost", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_SystemCosts_om_fuelcell_variable_cost_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
...
SAM_EXPORT void SAM_Equpartflip_LCOS_batt_annual_charge_energy_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "batt_annual_charge_energy", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_LCOS_batt_annual_charge_from_system_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "batt_annual_charge_from_system", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_LCOS_batt_annual_discharge_energy_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "batt_annual_discharge_energy", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_LCOS_batt_capacity_percent_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "batt_capacity_percent", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_LCOS_batt_salvage_percentage_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "batt_salvage_percentage", number);
	});
}

SAM_EXPORT void SAM_Equpartflip_LCOS_battery_total_cost_lcos_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "battery_total_cost_lcos", number);
	});
}

SAM_EXPORT void SAM_Equpartflip_LCOS_charge_w_sys_ec_ym_mset(SAM_table ptr, double* mat, int nrows, int ncols, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_matrix(ptr, "charge_w_sys_ec_ym", mat, nrows, ncols);
	});
}

SAM_EXPORT void SAM_Equpartflip_LCOS_grid_to_batt_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "grid_to_batt", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_LCOS_monthly_batt_to_grid_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "monthly_batt_to_grid", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_LCOS_monthly_grid_to_batt_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "monthly_grid_to_batt", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_LCOS_monthly_grid_to_load_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "monthly_grid_to_load", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_LCOS_monthly_system_to_grid_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "monthly_system_to_grid", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_LCOS_true_up_credits_ym_mset(SAM_table ptr, double* mat, int nrows, int ncols, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_matrix(ptr, "true_up_credits_ym", mat, nrows, ncols);
	});
}

SAM_EXPORT void SAM_Equpartflip_LCOS_year1_monthly_ec_charge_gross_with_system_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "year1_monthly_ec_charge_gross_with_system", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_LCOS_year1_monthly_ec_charge_with_system_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "year1_monthly_ec_charge_with_system", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_LCOS_year1_monthly_electricity_to_grid_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
...
SAM_EXPORT void SAM_Equpartflip_FuelCell_annual_fuel_usage_lifetime_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "annual_fuel_usage_lifetime", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_FuelCell_fuelcell_annual_energy_discharged_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "fuelcell_annual_energy_discharged", arr, length);
	});
}

SAM_EXPORT void SAM_Equpartflip_FuelCell_fuelcell_replacement_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
...
SAM_EXPORT double SAM_Equpartflip_Revenue_start_day_of_year_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "start_day_of_year", &result))
		make_access_error("SAM_Equpartflip", "start_day_of_year");
	});
	return result;
}
...
SAM_EXPORT double SAM_Equpartflip_FinancialParameters_batt_salvage_percentage_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "batt_salvage_percentage", &result))
		make_access_error("SAM_Equpartflip", "batt_salvage_percentage");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_SystemCosts_om_batt_capacity_cost_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "om_batt_capacity_cost", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "om_batt_capacity_cost");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_SystemCosts_om_batt_fixed_cost_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "om_batt_fixed_cost", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "om_batt_fixed_cost");
	});
	return result;
}

SAM_EXPORT double SAM_Equpartflip_SystemCosts_om_batt_nameplate_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "om_batt_nameplate", &result))
		make_access_error("SAM_Equpartflip", "om_batt_nameplate");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_SystemCosts_om_batt_replacement_cost_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "om_batt_replacement_cost", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "om_batt_replacement_cost");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_SystemCosts_om_batt_variable_cost_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "om_batt_variable_cost", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "om_batt_variable_cost");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_SystemCosts_om_fuelcell_capacity_cost_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "om_fuelcell_capacity_cost", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "om_fuelcell_capacity_cost");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_SystemCosts_om_fuelcell_fixed_cost_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "om_fuelcell_fixed_cost", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "om_fuelcell_fixed_cost");
	});
	return result;
}

SAM_EXPORT double SAM_Equpartflip_SystemCosts_om_fuelcell_nameplate_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "om_fuelcell_nameplate", &result))
		make_access_error("SAM_Equpartflip", "om_fuelcell_nameplate");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_SystemCosts_om_fuelcell_replacement_cost_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "om_fuelcell_replacement_cost", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "om_fuelcell_replacement_cost");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_SystemCosts_om_fuelcell_variable_cost_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "om_fuelcell_variable_cost", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "om_fuelcell_variable_cost");
	});
	return result;
}
...
SAM_EXPORT double SAM_Equpartflip_LandLease_land_area_nget(SAM_table ptr, SAM_error *err){
	...
	if (!ssc_data_get_number(ptr, "land_area", &result))
		make_access_error("SAM_Equpartflip", "land_area");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_LandLease_om_land_lease_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "om_land_lease");
	});
	return result;
}
...
SAM_EXPORT double SAM_Equpartflip_LandLease_om_land_lease_escal_nget(SAM_table ptr, SAM_error *err){
	...
	if (!ssc_data_get_number(ptr, "om_land_lease_escal", &result))
		make_access_error("SAM_Equpartflip", "om_land_lease_escal");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_SystemOutput_gen_purchases_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "gen_purchases");
	});
	return result;
}
...
SAM_EXPORT double SAM_Equpartflip_ElectricityRates_en_electricity_rates_nget(SAM_table ptr, SAM_error *err){
	...
	if (!ssc_data_get_number(ptr, "en_electricity_rates", &result))
		make_access_error("SAM_Equpartflip", "en_electricity_rates");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_ElectricityRates_rate_escalation_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "rate_escalation");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_UtilityBill_utility_bill_w_sys_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "utility_bill_w_sys");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_LCOS_batt_annual_charge_energy_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
...
SAM_EXPORT double* SAM_Equpartflip_LCOS_batt_annual_charge_from_system_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "batt_annual_charge_from_system", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "batt_annual_charge_from_system");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_LCOS_batt_annual_discharge_energy_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "batt_annual_discharge_energy", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "batt_annual_discharge_energy");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_LCOS_batt_capacity_percent_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "batt_capacity_percent", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "batt_capacity_percent");
	});
	return result;
}

SAM_EXPORT double SAM_Equpartflip_LCOS_batt_salvage_percentage_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "batt_salvage_percentage", &result))
		make_access_error("SAM_Equpartflip", "batt_salvage_percentage");
...
SAM_EXPORT double SAM_Equpartflip_LCOS_battery_total_cost_lcos_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "battery_total_cost_lcos", &result))
		make_access_error("SAM_Equpartflip", "battery_total_cost_lcos");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_LCOS_charge_w_sys_ec_ym_mget(SAM_table ptr, int* nrows, int* ncols, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_matrix(ptr, "charge_w_sys_ec_ym", nrows, ncols);
	if (!result)
		make_access_error("SAM_Equpartflip", "charge_w_sys_ec_ym");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_LCOS_grid_to_batt_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "grid_to_batt", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "grid_to_batt");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_LCOS_monthly_batt_to_grid_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "monthly_batt_to_grid", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "monthly_batt_to_grid");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_LCOS_monthly_grid_to_batt_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "monthly_grid_to_batt", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "monthly_grid_to_batt");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_LCOS_monthly_grid_to_load_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "monthly_grid_to_load", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "monthly_grid_to_load");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_LCOS_monthly_system_to_grid_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "monthly_system_to_grid", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "monthly_system_to_grid");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_LCOS_true_up_credits_ym_mget(SAM_table ptr, int* nrows, int* ncols, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_matrix(ptr, "true_up_credits_ym", nrows, ncols);
	if (!result)
		make_access_error("SAM_Equpartflip", "true_up_credits_ym");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_LCOS_year1_monthly_ec_charge_gross_with_system_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "year1_monthly_ec_charge_gross_with_system", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "year1_monthly_ec_charge_gross_with_system");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_LCOS_year1_monthly_ec_charge_with_system_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "year1_monthly_ec_charge_with_system", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "year1_monthly_ec_charge_with_system");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_LCOS_year1_monthly_electricity_to_grid_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "year1_monthly_electricity_to_grid", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "year1_monthly_electricity_to_grid");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_ChargesByMonth_net_billing_credits_ym_mget(SAM_table ptr, int* nrows, int* ncols, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "net_billing_credits_ym");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_ChargesByMonth_nm_dollars_applied_ym_mget(SAM_table ptr, int* nrows, int* ncols, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "nm_dollars_applied_ym");
	});
	return result;
}
...
SAM_EXPORT double SAM_Equpartflip_BatterySystem_en_standalone_batt_nget(SAM_table ptr, SAM_error *err){
	...
	if (!ssc_data_get_number(ptr, "en_standalone_batt", &result))
		make_access_error("SAM_Equpartflip", "en_standalone_batt");
	});
	return result;
}
...
SAM_EXPORT double SAM_Equpartflip_BatterySystem_en_wave_batt_nget(SAM_table ptr, SAM_error *err){
	...
	if (!ssc_data_get_number(ptr, "en_wave_batt", &result))
		make_access_error("SAM_Equpartflip", "en_wave_batt");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_FuelCell_annual_fuel_usage_lifetime_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "annual_fuel_usage_lifetime", length);
	if (!result)
...
SAM_EXPORT double* SAM_Equpartflip_FuelCell_fuelcell_annual_energy_discharged_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "fuelcell_annual_energy_discharged", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "fuelcell_annual_energy_discharged");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_FuelCell_fuelcell_replacement_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "fuelcell_replacement", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "fuelcell_replacement");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_annual_cost_lcos_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_annual_cost_lcos");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_annual_discharge_lcos_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_annual_discharge_lcos");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_charging_cost_grid_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_charging_cost_grid", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_charging_cost_grid");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_charging_cost_grid_month_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_charging_cost_grid_month", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_charging_cost_grid_month");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_charging_cost_pv_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_charging_cost_pv", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_charging_cost_pv");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_purchases_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_purchases");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_apr_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_apr", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_apr");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_aug_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_aug", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_aug");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_dec_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dec", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_dec");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_dispatch1_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dispatch1", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_dispatch1");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_dispatch2_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dispatch2", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_dispatch2");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_dispatch3_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dispatch3", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_dispatch3");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_dispatch4_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dispatch4", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_dispatch4");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_dispatch5_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dispatch5", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_dispatch5");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_dispatch6_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dispatch6", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_dispatch6");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_dispatch7_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dispatch7", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_dispatch7");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_dispatch8_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dispatch8", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_dispatch8");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_dispatch9_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_dispatch9", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_dispatch9");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_feb_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_feb", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_feb");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_jan_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_jan", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_jan");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_jul_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_jul", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_jul");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_jun_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_jun", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_jun");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_mar_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_mar", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_mar");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_may_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_may", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_may");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_monthly_firstyear_TOD1_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_monthly_firstyear_TOD1", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_monthly_firstyear_TOD1");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_monthly_firstyear_TOD2_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_monthly_firstyear_TOD2", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_monthly_firstyear_TOD2");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_monthly_firstyear_TOD3_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_monthly_firstyear_TOD3", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_monthly_firstyear_TOD3");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_monthly_firstyear_TOD4_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_monthly_firstyear_TOD4", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_monthly_firstyear_TOD4");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_monthly_firstyear_TOD5_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_monthly_firstyear_TOD5", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_monthly_firstyear_TOD5");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_monthly_firstyear_TOD6_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_monthly_firstyear_TOD6", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_monthly_firstyear_TOD6");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_monthly_firstyear_TOD7_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_monthly_firstyear_TOD7", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_monthly_firstyear_TOD7");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_monthly_firstyear_TOD8_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_monthly_firstyear_TOD8", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_monthly_firstyear_TOD8");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_monthly_firstyear_TOD9_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_monthly_firstyear_TOD9", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_monthly_firstyear_TOD9");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_nov_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_nov", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_nov");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_oct_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_oct", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_oct");
	});
	return result;
}

SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_sales_sep_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_energy_sales_sep", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_sales_sep");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_energy_without_battery_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_energy_without_battery");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_itc_fed_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_itc_fed");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_itc_fed_amount_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_itc_fed_amount");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_itc_fed_percent_amount_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_itc_fed_percent_amount");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_itc_sta_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_itc_sta");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_itc_sta_amount_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_itc_sta_amount");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_itc_sta_percent_amount_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_itc_sta_percent_amount");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_itc_total_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_itc_total");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_land_lease_expense_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_land_lease_expense");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_om_batt_capacity_expense_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_om_batt_capacity_expense", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_om_batt_capacity_expense");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_om_batt_fixed_expense_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_om_batt_fixed_expense");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_om_capacity1_expense_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_om_capacity1_expense");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_om_capacity2_expense_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_om_capacity2_expense");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_om_fixed1_expense_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_om_fixed1_expense");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_om_fixed2_expense_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_om_fixed2_expense");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_om_production1_expense_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_om_production1_expense");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_om_production2_expense_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_om_production2_expense");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_salvage_cost_lcos_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "cf_salvage_cost_lcos", length);
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_salvage_cost_lcos");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_util_escal_rate_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_util_escal_rate");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Equpartflip_Outputs_cf_utility_bill_aget(SAM_table ptr, int* length, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_Equpartflip", "cf_utility_bill");
	});
	return result;
}
...
SAM_EXPORT double SAM_Equpartflip_Outputs_lcos_nom_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "lcos_nom", &result))
		make_access_error("SAM_Equpartflip", "lcos_nom");
	});
	return result;
}

SAM_EXPORT double SAM_Equpartflip_Outputs_lcos_real_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "lcos_real", &result))
		make_access_error("SAM_Equpartflip", "lcos_real");
	});
	return result;
}
...
SAM_EXPORT double SAM_Equpartflip_Outputs_npv_annual_costs_lcos_nget(SAM_table ptr, SAM_error *err){
	...
	if (!ssc_data_get_number(ptr, "npv_annual_costs_lcos", &result))
		make_access_error("SAM_Equpartflip", "npv_annual_costs_lcos");
	});
	return result;
}

SAM_EXPORT double SAM_Equpartflip_Outputs_npv_energy_lcos_nom_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "npv_energy_lcos_nom", &result))
		make_access_error("SAM_Equpartflip", "npv_energy_lcos_nom");
	});
	return result;
}

SAM_EXPORT double SAM_Equpartflip_Outputs_npv_energy_lcos_real_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "npv_energy_lcos_real", &result))
		make_access_error("SAM_Equpartflip", "npv_energy_lcos_real");
	});
	return result;
}
...
SAM_EXPORT double SAM_Equpartflip_Outputs_pre_depr_alloc_basis_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "pre_depr_alloc_basis", &result))
		make_access_error("SAM_Equpartflip", "pre_depr_alloc_basis");
	});
	return result;
}

SAM_EXPORT double SAM_Equpartflip_Outputs_pre_itc_qual_basis_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "pre_itc_qual_basis", &result))
		make_access_error("SAM_Equpartflip", "pre_itc_qual_basis");
	});
	return result;
}
